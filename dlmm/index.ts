...
import { IDL } from "./idl";
import { IDL as PumpForkingMeteoraSc } from "../target/types/pump_forking_meteora_sc";
import {
  BASIS_POINT_MAX,
  FEE_PRECISION,
  LBCLMM_PROGRAM_IDS,
...
export class DLMM {
  ...
  public static async getPairPubkeyIfExists(
    ...
    try {
      ...
      console.log("pool data---->", account2);
  ...
  public static async createLbPair(
    ...
    console.log(3)
    console.log(opt.programId)
    const program = new Program(
      PumpForkingMeteoraSc as Idl,
      opt.programId,
      provider
    );
    console.log(program.programId.toBase58())
    const dlmmPg = new Program(
      IDL,
    ...
    console.log("🚀 ~ DLMM ~ existsPool:", existsPool)
    if (existsPool) {
      console.log("exit")
    ...
    const [reserveX] = deriveReserve(tokenX, lbPair, dlmmPg.programId);
    const [reserveY] = deriveReserve(tokenY, lbPair, dlmmPg.programId);
    const [oracle] = deriveOracle(lbPair, dlmmPg.programId);

    const activeBinArrayIndex = binIdToBinArrayIndex(activeId);
    const binArrayBitmapExtension = isOverflowDefaultBinArrayBitmap(
      activeBinArrayIndex
    )
      ? deriveBinArrayBitmapExtension(lbPair, dlmmPg.programId)[0]
      : null;
    ...
    return program.methods
      .createMeteoraPool(activeId.toNumber(), binStep.toNumber())
      .accounts({
        lbPair,
        binArrayBitmapExtension,
        tokenMintX: tokenX,
    ...
    while (count >= 0) {
      ...
      if (binArray) {
        ...
        if (bins.every(({ amountX }) => amountX.isZero())) {
          ...
          let lastBinWithLiquidityIndex;
          for (let i = bins.length; i >= 0; i--) {
            if (!bins[i].amountX.isZero()) {
